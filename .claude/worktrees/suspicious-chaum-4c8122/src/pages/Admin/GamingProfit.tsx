...
import { getGamingProfit, ProfitDto } from '../../services/profitService';
...
export default function GamingProfit() {
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            try {
                ...
                // Filter only game categories (type = 'game')
                const gameCategories = res.data.filter(cat => cat.type === 'game');
    ...
    useEffect(() => {
        const fetchProfit = async () => {
            try {
                ...
                const data = await getGamingProfit({
                ...
                const message = err && typeof err === 'object' && 'message' in err
                    ? String(err.message)
                    : 'Failed to load gaming profit data';
                setError(message);
            } finally {
                setLoading(false);
            }
    ...
    return (
        <>
            <PageMeta title="Gaming Profit - AXIS" description="Gaming profit analysis" />
            <div className="p-6">
                <h1 className="text-2xl font-semibold mb-6">Gaming Profit</h1>
